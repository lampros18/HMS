	...
	for (let i = 0; i < $('[data-toggle="popover"]').length; i++) {
		if (document
				.getElementById("password"
						+ $('[data-toggle="popover"]')[i].parentNode.parentNode.parentNode.id) != null) {
			popoverData[$('[data-toggle="popover"]')[i].parentNode.parentNode.parentNode.id] = document
					.getElementById("password"
							+ $('[data-toggle="popover"]')[i].parentNode.parentNode.parentNode.id).value;
		}
	}
}
...
function loadTableAndData(response) {
	...
	for (let i = 0; i < json.users.length; i++) {
		...
		if (json.users[i].student == 1)
			createStudentAuthoritiesColumn(json.users[i].student,
					json.users[i].id);
		else
			createEmployeeAuthoritiesColumn(json.users[i].admin,
					json.users[i].foreman, json.users[i].employee,
					json.users[i].id);
...
function createTableRow(id, tableBody) {
	...
	tableRow.setAttribute("filtered", "no");
...
function createActionsRow(id, tableBody) {
	...
	iPasswordA.setAttribute("data-content",
			"<input data-input='input' id='password" + id + "' type='text'/>");
...
function createEmployeeAuthoritiesColumn(admin, foreman, employee, id) {
	...
	foreman_div.setAttribute("class", "custom-switch switch_input_span");
	...
	employee_div.setAttribute("class", " custom-switch switch_input_span");
...
document.getElementById('reload-pass').addEventListener('click', function() {
	createPassword()
});
...
document
		...
		.addEventListener(
				"click",
				function() {
					document.getElementById("employee_email").setAttribute(
							"class", "form-control");
					document.getElementById("employee_password").setAttribute(
							"class", "form-control");
					document.getElementById("type_employee").setAttribute(
							"class", "has-val");
					document.getElementById("type_student").setAttribute(
							"class", "has-val");
					document.getElementById("employee_authority_foreman")
							.setAttribute("class", "has-val");
					document.getElementById("employee_authority_admin")
							.setAttribute("class", "has-val");
					document.getElementById("employee_authority_employee")
							.setAttribute("class", "has-val");

					if (document.getElementById("employee_email").value.trim() == "") {
						document.getElementById("employee_email").setAttribute(
					...
					if (document.getElementById("employee_password").value
						...
						document.getElementById("employee_password")
								.setAttribute("class", "form-control error");
						document.getElementById("exampleModalLabel")
								.scrollIntoView();
						return;
					}
...
function deleteUser(uid) {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (json.status == 200)
				document.getElementById(uid).parentNode.removeChild(document
						.getElementById(uid));
			setResultMessage(xhttp.responseText);
		}
	};
		...
		xhttp
				.send(JSON
						.stringify({
							id : uid,
							username : $("#" + uid).find("td:eq(1)").text(),
							adminAuthority : document
									.getElementById("admin_switch" + uid).checked ? 1
									: 0,
							foremanAuthority : document
									.getElementById("foreman_switch" + uid).checked ? 1
									: 0,
...
function adminSwitch(id) {
	if (!document.getElementById("admin_switch" + id).checked) {
		if (!document.getElementById("foreman_switch" + id).checked
				&& !document.getElementById("employee_switch" + id).checked) {
...
function employeeSwitch(id) {
	if (!document.getElementById("employee_switch" + id).checked) {
		if (!document.getElementById("foreman_switch" + id).checked
				&& !document.getElementById("admin_switch" + id).checked) {
